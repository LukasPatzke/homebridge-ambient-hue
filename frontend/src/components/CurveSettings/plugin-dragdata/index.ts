...
type OnDrag = (e: MouseEvent, datasetIndex: number, index: number, value: Point) => void | boolean;
type OnDragEnd = (e: MouseEvent, datasetIndex: number, index: number, value: Point) => void;
type onDragStart = (e: MouseEvent, element: any, index: number, value: Point) => void | boolean;
...
const getElement = (e: MouseEvent, chartInstance: Chart<'scatter', Point[]>, callback?: onDragStart) => {
  ...
  if (element) {
    ...
    if ((datasetMeta.xAxisID === undefined) || (datasetMeta.yAxisID === undefined)) {
      element = null;
      return;
    }
...
function calcPosition(e: MouseEvent | TouchEvent, chartInstance: Chart<'scatter', Point[]>, datasetIndex: number, index: number) {
  let x, y;
  const dataset = chartInstance.data.datasets[datasetIndex];
  ...
  if (!dataPoint.first && !dataPoint.last && (x < dataset.data[index + 1].x) && (x > dataset.data[index - 1].x)) {
    dataPoint.x = x;
  }
...
const updateData = (e: MouseEvent, chartInstance: Chart<'scatter', Point[]>, callback?: OnDrag) => {
  if (element) {
    ...
    if (!callback || (callback(e, curDatasetIndex, curIndex, dataPoint) !== false)) {
...
const dragEndCallback = (e: MouseEvent, chartInstance: Chart<'scatter', Point[]>, callback?: OnDragEnd) => {
  curIndex = undefined;
  ...
  afterInit: function (chartInstance: Chart<'scatter', Point[]>) {
    if (chartInstance.config.options?.plugins && chartInstance.config.options.plugins.dragData) {
      ...
      select(chartInstance.canvas).call(
        drag<HTMLCanvasElement, unknown>().container(chartInstance.canvas)
          .touchable(() => pluginOptions.enableTouch || false)
          .on('start', e => getElement(e.sourceEvent, chartInstance, pluginOptions.onDragStart))
          .on('drag', e => updateData(e.sourceEvent, chartInstance, pluginOptions.onDrag))
